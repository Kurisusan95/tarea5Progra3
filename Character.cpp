...
Move* Character::getMove(SDL_Renderer *renderer, string name, int sprite_amount)
{
    ...
    for(int i=1;i<=sprite_amount;i++)
    {
        ...
        string hurtbox_file_path = "assets/" + name + "/hurtboxes/"+toString(i)+".txt";
        ifstream hurtbox_file(hurtbox_file_path.c_str());

        while(hurtbox_file>>x)
        {
            hurtbox_file>>y;
            hurtbox_file>>w;
            hurtbox_file>>h;
            hurtboxes.push_back(new Hurtbox(x,y,w,h));
        }
...
vector<Hurtbox*> Character::getHurtboxes()
{
